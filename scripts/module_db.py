...
class ModuleDb:
    ...
    def get_modules_enabled_names(self) -> [str]:
        em: [str] = [module.name for module in self.get_modules() if module.build]
        em.sort()
        return em

    def get_modules_disabled_names(self) -> [str]:
        dm: [str] = [module.name for module in self.get_modules() if not module.build]
        dm.sort()
        return dm

    def get_module_build_paths(self) -> [str]:
        bp: [str] = [module.path for module in self.get_modules_dependency_sorted() if module.build]
        return bp
...
def __parse_module_config(config_path: str, env: Environment) -> Module:
    ...
    if "get_name" in config:
        ...
        module_name = os.path.basename(os.path.dirname(config_path))
        ...
        class CompatDict:
            def __init__(self, env):
                self.d = {
                    ...
                    "disable_3d": False, # TODO
                    "bits": "64" if "64" in env.cpu_family else "32",
                    "arch": env.cpu_family,
                    "android_arch": env.cpu_family,
                }
    ...
    if not os.path.isfile(build_file):
        module.build = False
        module.build_info.append("The meson.build file for this module is missing.")
    ...
    def check_deps(module: Module) -> bool:
        ...
        for dep_name, dep_options in module.module_dependencies.items():
            ...
            if not mdb.has_module(dep_name):
                module.build = False
                module.build_info.append("Dependent module " + dep_name + " not found.")
                return False

            # Check if the module we refer to has its dependencies met
            elif not check_deps(mdb.get_module(dep_name)) and required:
                module.build = False
                module.build_info.append('Cannot build module due do dependency "' + dep_name + '" not building.')
...
def __create_db_file(args):
    ...
    for module in mdb.get_modules():
        if module.name in modules_disabled:
            module.build = False
            module.build_info.append("Module was disabled from the command line.")
...
if __name__ == "__main__":
    ...
    create_db_parser = subparsers.add_parser("create_db", help="create_db help")
    ...
    create_db_parser.add_argument(
        ...
        dest="tools_enabled",
        action="store_true",
        help="Whether or not tooling is enabled for this build.",
    )
    create_db_parser.add_argument(
        "--module_search_path",
        type=str,
        ...
        action="append",
        help=(
            "The module search paths to check for config.py's\n"
    ...
    create_db_parser.add_argument(
        ...
        action="append",
        help=(
            "The modules to be disabled for building.\n"
    ...
    modules_enabled_parser = subparsers.add_parser("get_enabled_modules", help="get_enabled_modules help")
    modules_enabled_parser.add_argument("database_file", type=str, help="The input modules_db file")

    # modules disabled printer
    modules_disabled_parser = subparsers.add_parser("get_disabled_modules", help="get_disabled_modules help")
    modules_disabled_parser.add_argument("database_file", type=str, help="The input modules_db file")

    # module buld_files printer
    modules_bp_parser = subparsers.add_parser("get_module_build_paths", help="get_module_build_paths help")
    modules_bp_parser.add_argument("database_file", type=str, help="The input modules_db file")
